...
exports.createStreamAndToken = (req, res, next) => {
  ...
  const { privilegeExpiredTs, rtcToken } = rtcTokenGenerator("model", req.user.relatedUser._id.toString(), req.user.relatedUser._id.toString())
  ...
  Stream({
    ...
    .then(stream => {
      ...
      return Model.findOneAndUpdate({ _id: req.user.relatedUser._id },
        ...
        .select("name userName isStreaming onCall age gender languages profileImages rating dob") /* for live updating of the landing page */
        .lean()
    })
    .then(model => {
  ...
  Model.findById(modelId, "currentStream isStreaming onCall")
    ...
    .then(model => {
      ...
      if (model.isStreaming) {
         ...
         * and being tracked
         */
        return UnAuthedViewer.findOneAndUpdate({ _id: unAuthedUserId }, {
          $inc: {
            "sessions": 1,
            "streamViewed": 1
          },
