...
exports.createTag = (req, res, next) => {
    controllerErrorCollector(req, "Improper data given, please input data in correct format and try again!")
...
exports.updateTagGroup = (req, res, next) => {
    ...
    TagGroup.findOneAndUpdate({ _id: id }, {
        name: name,
        description: description,
        tags: tagIds ? tagIds : []
    },{new:true})
            ...
            return Tag.count()
                .then(count => {
                    if (tags !== null && tags.length !== 0) {
                        ...
                        const error = new Error("Exceeded limit, this much tags does not exist")
            ...
            return TagGroup.count()
                .then(count => {
                    if (tagGroups !== null && tagGroups.length !== 0) {
                        ...
                        const error = new Error("Exceeded limit, this much tagGroups does not exist")
                        error.statusCode = 400
                        throw error
                    }
                })
